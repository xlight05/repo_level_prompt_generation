...
public class ClassInfo {
	...
	public static ClassInfo parse(Class cls) {
		try {
			...
			return new ClassInfo(cls.getName(), s);
		} catch (Exception e) {
	...
	private ClassInfo(String className, InputStream s) throws Exception {
	 ...
	 * @return
	 */
	public MemberInfo getBeanPathMember(String name) {
		String[] members = name.split("\\.");
		ClassInfo clsInfo = this;
		MemberInfo mi = clsInfo.getBeanMember(members[0]);

		for (int i = 1; i < members.length; i++) {
			try {
				Class cls = Class.forName(mi.getType());
				clsInfo = ClassInfo.parse(cls);
				mi = clsInfo.getBeanMember(members[i]);
				if (mi == null)
					return null;
			} catch (ClassNotFoundException e) {
				throw new RuntimeException("Could not find class info for "
						+ name, e);
			}
		}
	...
	public MemberInfo getBeanMember(String name) {
		...
		FieldInfo info = getField(name);
		if (info != null)
			return info;

		ClassInfo parent = this.getParent();
		if (parent != null)
			return parent.getBeanMember(name);
		else
			return null;
	}
	...
	public ClassInfo getParent() {
		...
		ConstPoolEntry classEntry = (ConstPoolEntry) this.constantPool
				.get(this.superClassIndex);
		ConstPoolEntry classNameEntry = (ConstPoolEntry) this.constantPool
				.get(classEntry.index1);
		InputStream s = ClassInfo.class.getResourceAsStream("/"
				+ classNameEntry.getValueAsString() + ".class");
		try {
			return new ClassInfo(classNameEntry.getValueAsString(), s);
		} catch (Exception e) {
			throw new RuntimeException("Could not parse class file: "
					+ classNameEntry.getValueAsString(), e);
		}
	}
	...
	public String toString() {
		...
		if (this.superClassIndex != 0) {
			...
			ConstPoolEntry classNameEntry = (ConstPoolEntry) this.constantPool
					.get(classEntry.index1);
			output = "SuperClass = " + classNameEntry.getValueAsString();
		} else
			output = "SuperClass = none";

		output += "\n\nNumber of Fields: " + this.fieldCount + "\n"
				+ "Fields\n";
