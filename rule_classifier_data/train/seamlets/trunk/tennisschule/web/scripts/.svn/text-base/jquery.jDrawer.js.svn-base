...
jQuery.fn.extend(
{
	jDrawer: function(settings)
	{
		...
		settings = jQuery.extend({}, jQuery.jDrawer.settings, settings);
		...
		//horizontal bug fix
		jQuery("ul.jDrawer-list", self).css({height: self.css("height"), width: self.css("width")});

		jQuery("li.jDrawer-item", self).each(function()
		{
			var current = jQuery(this);
		...
		if(settings.layout === "vertical")
		{
			...
			Resize = function()
			{
				...
				jQuery("ul.jDrawer-list", self).height(tH);
			};
			
			Slide = function(active)
			{
				jQuery("li.jDrawer-active", self).removeClass("jDrawer-active");
		...
		else if(settings.layout === "horizontal")
		{
			...
			Resize = function()
			{
				...
				jQuery("ul.jDrawer-list", self).width(tW);
			};
		...
		Colorize = function()
		{
			...
			jQuery("#" + self.attr("id") + ", #" + self.attr("id") + " .jDrawer-content", self.parent()).css("background-color", color);
		};
		...
		var active = jQuery(".jDrawer-active", self);
