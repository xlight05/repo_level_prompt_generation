		...
		try {
			...
			if (auctions != null) {
				for (Entry<String, Object> entry : auctions.entrySet()) {
					final int auctionId = Integer.parseInt(entry.getKey());
					Object valueObj = entry.getValue();
					if (valueObj instanceof Map) {
						Map<String, Object> auction = (Map<String, Object>) valueObj;
						final Object statusObj = auction.get("s");
						if (statusObj != null && "Completed".equals(statusObj)) {
							listener.handleAuctionSold(auctionId);
						}
