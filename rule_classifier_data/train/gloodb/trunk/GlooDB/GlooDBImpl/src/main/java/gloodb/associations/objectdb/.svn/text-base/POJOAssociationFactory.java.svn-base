...
public class POJOAssociationFactory extends AssociationFactory {
     ...
     */
    @Override
    public <T extends Serializable, V extends Serializable> NonUniqueIndex<T, V> newNonUniqueIndex(Class<T> tClass, String name, Comparator<V> comparator, Serializable... parameters) {
        return new NonUniqueIndexImplementation<T, V>(tClass, name, comparator, parameters);
    }

    /**
     * Returns a pojo based many to many association.
     */
	@Override
	public <L extends Serializable, R extends Serializable> ManyToManyAssociation<L, R> newManyToManyAssociation(Class<L> lClass, Class<R> rClass) {
		return new ManyToManyAssociationImplementation<L, R>(lClass, rClass);
	}
}
