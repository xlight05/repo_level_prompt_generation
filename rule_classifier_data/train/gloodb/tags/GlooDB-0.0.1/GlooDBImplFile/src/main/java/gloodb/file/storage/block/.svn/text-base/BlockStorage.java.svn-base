...
class BlockStorage implements Storage {
    ...
    private int writeBlock(byte[] buf, int blockNo, int pos) throws IOException {
        ...
        file.seek(pos);
        if (blockNo > 0) {
            // Optimization; write with a single I/O
            buf[blockNo * writableBlockSize - 1] = 1;
            file.write(buf, blockNo * writableBlockSize - 1, size + 1);
        } else {
     ...
     * @param persistentObject The persitent object.
	...
	public void start(BigInteger startVersion) {
		// TODO Backup the current storage file
