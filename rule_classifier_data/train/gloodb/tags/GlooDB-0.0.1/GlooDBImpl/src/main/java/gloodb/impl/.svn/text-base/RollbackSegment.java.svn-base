...
class RollbackSegment {
    ...
    void startTx(Transaction tx) {
        this.rollbackSegment.put(tx, new LinkedList<UndoTransaction>());
    }
     ...
     */
    void addUndoTx(Transaction tx, UndoTransaction rollbackTx) {
     ...
     */
    void undoTx(TxContext ctx, Transaction tx, ObjectMap objectWarehouse) {
        List<UndoTransaction> rollbackTxs = this.rollbackSegment.get(tx);
	...
	public boolean containsObjectBackup(Serializable identity) {
		return (this.objectBackups.get(identity) != null);
	}
	
    void tryLock(Transaction tx, Serializable id) {
    ...
    boolean isLockedByAncestor(Transaction lockingTx, Transaction tx) {
        Transaction parentTx = tx.getParentTx();
        if (parentTx == null) {
            return false;
        }
