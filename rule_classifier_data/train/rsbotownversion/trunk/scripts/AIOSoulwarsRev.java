/** 
 * AIOSoulwars by Zenzie, modified by ZDScripting, who then tried to make it a paid script
 * Debugged by David2 and re-released for public use. Enjoy!
 */
...
@ScriptManifest(authors = { "Zenzie, Zeus, David2" }, category = "Mini-Game", name = "AIOSoulwars 1.2", version = 1.2, description = "<html><head>"
		...
		+ "<h1>AIOSoulwars 1.2!</h1>"
		+ "<h2>Zenzie ed. Zeus, David2</h2></div><br />"
		+ "Both graveyards should work now."
		+ "Make sure that you are already using the combat style that you want to train."
		+ "Make sureee you are starting it by the green portal!"
		...
		+ "<option>Clan chat *not working*</option>"
		...
		+ "<hr>"
		+ "<h2> News! </h2>"
		+ "Script has been released for public use under the GPL<p>"
		+ "Attack NPCs untested, all join options flawless." + "</body></html>")
public class AIOSoulwarsRev extends Script implements PaintListener,
		ServerMessageListener {

	public enum actions {
		CHOOSING_TEAM, JOINING_BLUE, JOINING_RED, WAITING_FOR_GAME, LEAVING_SPAWN, WALKING_TO_BANDAGES, GETTING_BANDAGES, USING_BANDAGES, ATTACKING_PLAYERS, LEAVING_GRAVEYARD, ATTACKING_MONSTER, WAIT
	}
	...
	public void onRepaint(Graphics g) {
		...
		case 2:
			...
			g.drawString("AIOSoulwars V1.2", 560, 435);
			g.drawString("By Zenzie, ed. Zeus/David2", 560, 450);
			break;
		}
	}
	...
	public actions getAction() {
		...
		if (actionName.equals("Choosing Team")
				&& !actionName.equals("Waiting for game") && !inGame) {
			return actions.CHOOSING_TEAM;
		}
		if (actionName.contains("Joining")
				&& !actionName.equals("Choosing Team")
				&& !inArea(1870, 1879, 3158, 3166)
				&& !inArea(1900, 1909, 3157, 3166)
				&& !inArea(1816, 1823, 3220, 3230)
				&& !inArea(1951, 1958, 3234, 3244) && !inGame) {
		...
		if ((inArea(1798, 1811, 3250, 3260) || inArea(1963, 1974, 3200, 3212))
				&& howToPlayInGame.equals("Bandage") && inGame) {
		...
		if (!inArea(1816, 1823, 3220, 3230) && !inArea(1951, 1958, 3234, 3244)
				&& !inArea(1798, 1811, 3250, 3260)
				&& !inArea(1963, 1974, 3200, 3212)
				&& howToPlayInGame.equals("Bandage") && inGame) {
			return actions.WALKING_TO_BANDAGES;
		}
		if (howToPlayInGame.equals("Attack Players") && inGame) {
			if (!inArea(1841, 1843, 3217, 3219)
					&& !inArea(1932, 1934, 3244, 3246)
					&& !inArea(1816, 1823, 3220, 3230)
					&& !inArea(1951, 1958, 3234, 3244)) {
				return actions.ATTACKING_PLAYERS;
			}
		}
		if ((inArea(1841, 1843, 3217, 3219) || inArea(1932, 1934, 3244, 3246))
				&& inGame) {
			return actions.LEAVING_GRAVEYARD;
		}
		if (!inArea(1816, 1823, 3220, 3230) && !inArea(1951, 1958, 3234, 3244)
				&& !inArea(1841, 1843, 3217, 3219)
				&& !inArea(1932, 1934, 3244, 3246)
				&& howToPlayInGame.equals("Attack NPCs") && inGame) {
	...
	public int loop() {
		...
		if (!actionName.equals("Choosing Team")
				&& !inArea(1770, 2000, 3180, 3610)
				&& !actionName.contains("Joining")
				&& !inArea(1870, 1879, 3158, 3166)
				&& !inArea(1900, 1909, 3157, 3166)) {
			inGame = false;
			actionName = "Choosing Team";
		...
		if (time.equals("2 mins") || time.equals(" 1 min")
				|| time.equals("3 mins")) {
			if (whatToJoin.equals("Clan chat")) {
				continueChat = true;
			}
		...
		case CHOOSING_TEAM:
			...
			} else if (whatToJoin.equals("Clan chat") && callBlue == 0
					&& callRed == 0) {
				continueChat = true;
		...
		case GETTING_BANDAGES:
			...
			if (onTeam.equals("Blue")) {
				...
				if (bandageTableBlue != null) {
					if (distanceTo(new RSTile(1810, 3254)) > 2
					...
					if (distanceTo(bandageTableBlue) <= 2
						...
						if (BandagesClickByClick.equals("No")) {
							...
							if (RSInterface.getInterface(752).getChild(4)
									.isValid()
									&& RSInterface.getInterface(752)
											.getChild(4).containsText(
													"How many bandages would")
									&& !getMyPlayer().isInCombat()) {
								if (distanceTo(bandageTableBlue) <= 2) {
									sendText(Integer.toString(random(28, 100)),
											true);
								}
								wait(random(600, 950));
							}
				...
				if (bandageTableRed != null) {
					if (distanceTo(new RSTile(1963, 3207)) > 2
							&& !getMyPlayer().isMoving()) {
		...
		case USING_BANDAGES:
			if (turnOnQuickPrayAt.equals("Bandages")) {
				if (getSetting(1396) == 0
						&& skills.getCurrentSkillLevel(Constants.STAT_PRAYER) > 0) {
		...
		case ATTACKING_PLAYERS:
			...
			if (inArea(1872, 1901, 3218, 3245)) {
				...
				if (turnOnQuickPrayAt.equals("Obelisk")) {
					if (getSetting(1396) == 0
							&& skills
									.getCurrentSkillLevel(Constants.STAT_PRAYER) > 0) {
						atInterface(749, 4, "Turn quick prayers on");
					}
				}
		...
		case ATTACKING_MONSTER:
			...
			if (slayerLevel >= 29 && slayerLevel <= 59
					&& !inArea(1835, 1855, 3241, 3256)
					&& !inArea(1920, 1935, 3206, 3218)) {
				actionName = "Walking to pyres";
			}
			if (slayerLevel >= 60 && !inArea(1874, 1893, 3203, 3216)
					&& !inArea(1887, 1908, 3248, 3255)) {
	...
	// Creds to Peach
	public boolean clickTheCharacter(RSCharacter npc, String npcName,
			String action) {
		if (npc == null) {
			return false;
		}
	...
	public boolean walkTo2(RSTile dest) {
		...
		if (!getMyPlayer().isMoving()
				|| distanceTo(getDestination()) < random(5, 10)
				&& !isRandDest(3, dest)) {
			if (getMyPlayer().getInteracting() == null) {
				return walkTo(dest);
			}
	...
	public int waitTime() {
		if (RSInterface.getChildInterface(211, 1).isValid()
				&& RSInterface.getChildInterface(211, 1).containsText(
	...
	public boolean useBandagesOnOtherPlayer() {
		...
		for (int element : validPlayers) {
			...
			try {
				...
				if (distanceTo(bandaging) > 10 || !pointOnScreen(checkPlayer)
				...
				if (onTeam.equals("Blue")) {
					...
					if (bandageTableBlue != null) {
						if (distanceTo(bandageTableBlue.getLocation()) >= 8
								&& !getMyPlayer().isMoving()) {
					...
					if (bandageTableRed != null) {
						if (distanceTo(bandageTableRed.getLocation()) >= 8
								&& !getMyPlayer().isMoving()) {
				...
				if (inventoryContains(14640)
						&& getActivityBarPercent() <= random(40, 80)
						&& player.getTeam() == getMyPlayer().getTeam()) {
					if (!isItemSelected()) {
	...
	public boolean atMenuThingBandages(int i) {
		...
		try {
			...
			final int xOff = random(4,
					getMenuItems().get(randomIdx).length() * 4);
	...
	public class clanChat extends Thread {
		...
		private String getLastClanChatMessage() {
			for (int i = 155; i >= 56; i--) {
				String text = RSInterface.getInterface(INTERFACE_CHAT_BOX)
						.getChild(i).getText();
