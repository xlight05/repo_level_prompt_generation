...
@ScriptManifest(authors = { "Fallen" }, category = "Magic", name = "Fallen's Superheater", version = 4.0,
...
if(Option == 1) {
		...
		if(DoTheMath == true) {
			...
			g.drawString("" + BarType + " bars/Hour: " + XPHRM / 53, 387, 270);
		}
}
if(Option == 2) {
		...
		if(DoTheMath == true) {
			...
			g.setFont(new Font("Verdana", 0, 9));
			g.drawString("" + BarType + " bars/Hour: " + XPHRM / 53, 387, 214);
		}
		//Magic
		g.setFont(new Font("Verdana", 0, 10));
		g.setColor(Color.CYAN);
		g.drawString("Magic level: " + currentLevelM + " (" + levelsGainedM + ")", 20, 256 );
		g.drawString("Magic EXP gained: " + (BarCounter * 53), 20, 271);
		//Smithing
		g.setColor(Color.WHITE);
		g.drawString("Smithing level: " + currentLevelS + " (" + levelsGainedS + ")", 193, 256 );
		g.drawString("Smith EXP gained: " + (BarCounter * BarEXP), 193, 271);
		//Bars & Profit
		if(DoTheMath == true) {
			g.setColor(Color.YELLOW);	
			g.drawString("" + BarType + " bars/Hour: " + XPHRM / 53, 360, 241);
			g.drawString("Bars made: " + BarCounter + " " + BarType, 360, 256);		
			g.drawString("Total profit: " + BarCounter * + profit, 360, 271);		
		} else {
			g.setColor(Color.YELLOW);
			g.drawString("Bars made: " + BarCounter + " " + BarType, 360, 271);	
		}
}
	//VERSION & AMOUNT OF CASTS
		...
		if (AmountOfCasts != 0) {
			...
			g.drawString("Casts left: " + ((AmountOfCasts) - (BarCounter)), 365, 57);
		} else {
			g.setFont(new Font("Verdana", 0, 14));
			g.setColor(Color.WHITE);
			g.drawString("Time: " + hours + ":" + minutes + ":"
		...
		//PROGGY BARS
			...
			g.setColor(Color.WHITE);
			g.drawString(""+toNextLvlS + "% to "+nextLvlS+" Smithing", 56, 320);
	...
	public boolean onStart(final Map<String, String> args) {
		try {
			BKG = ImageIO.read(new URL("http://img41.imageshack.us/img41/3554/newpaintbkg2.png"));
			//Timer = ImageIO.read(new URL("http://img535.imageshack.us/img535/9061/originaltimer.png"));
			Timer = ImageIO.read(new URL("http://img135.imageshack.us/img135/9061/originaltimer.png"));
