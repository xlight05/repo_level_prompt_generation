...
@ScriptManifest(authors = { "Jacmob" }, category = "Crafting", name = "Epic Auto Flax Spinner", version = 1.31, description = "<html><head><style type=\"text/css\">body { font-family: Arial; padding: 10px; }</style></head><body><strong>AutoSpinner V1.31 | Jacmob</strong><br /><br />Start the script in Lumbridge Castle with flax visible at the top of your bank.</body></html>")
public class EpicAutoFlaxSpinner extends Script implements PaintListener {
	...
	public int loop() {
		...
		case WALKTOBANK:
			if (SPIN_INTERFACE.isValid()) {
				clickMouse(random(121, 198), random(391, 452), true);
			}
			if (tileOnScreen(STAIRCASE_TILE)) {
	...
	public void onRepaint(final Graphics g) {
		if (isLoggedIn()
			...
			g.setColor(TEXT);
			g.drawString("Epic Auto Flax Spinner v1.2", x, y += 17);
